...
# Color palette — defaults. Brand colors from the audited site override the
# "chrome" colors (primary/accent/highlight) via apply_brand_palette(); the
# semantic score colors (success/warning/danger) stay fixed so good/bad always
# reads the same regardless of brand.
COLORS = {
...
def _norm_hex(value):
    """Return a valid #rrggbb string or None. Accepts #rgb, #rrggbb, with/without #."""
...
def _blend(hex_color, with_color="#FFFFFF", ratio=0.92):
    """Blend hex_color toward with_color by ratio (0=keep, 1=fully with_color)."""
    h = hex_color.lstrip("#")
    w = with_color.lstrip("#")
    r1, g1, b1 = int(h[0:2], 16), int(h[2:4], 16), int(h[4:6], 16)
    r2, g2, b2 = int(w[0:2], 16), int(w[2:4], 16), int(w[4:6], 16)
...
def apply_brand_palette(brand_colors):
    """Override the report's chrome colors with the analyzed brand's colors.

    brand_colors: {"primary": "#hex", "accent": "#hex", "highlight": "#hex"} — all optional.
    Falls back to defaults for anything missing or invalid. Derives a soft tinted
    background and border from the accent so the whole report feels on-brand.
    """
    brand_colors = brand_colors or {}
    primary = _norm_hex(brand_colors.get("primary"))
    accent = _norm_hex(brand_colors.get("accent"))
    highlight = _norm_hex(brand_colors.get("highlight"))

    if primary:
        COLORS["primary"] = HexColor(primary)
    if accent:
        COLORS["accent"] = HexColor(accent)
    if highlight:
        COLORS["highlight"] = HexColor(highlight)

    # Tinted background + border derived from accent (or primary) for a branded feel.
    tint_source = accent or primary
    if tint_source:
        COLORS["light_bg"] = HexColor(_blend(tint_source, "#FFFFFF", 0.93))
        COLORS["border"] = HexColor(_blend(tint_source, "#FFFFFF", 0.78))
...
def generate_report(data, output_path):
    ...
    # Apply the audited brand's colors before building any styles.
    apply_brand_palette(data.get("brand_colors"))
    ...
    # Branded color band — primary block + accent stripe, in the audited brand's colors.
    band = Drawing(500, 14)
    band.add(Rect(0, 4, 300, 10, fillColor=COLORS["primary"], strokeColor=None))
    band.add(Rect(300, 4, 200, 10, fillColor=COLORS["accent"], strokeColor=None))
    ...
    elements.append(Spacer(1, 0.5 * inch))
    elements.append(Paragraph(
        "Generated by your AI Marketing Team for Claude Code",
